...
# dataCSC  = librosa.load(file_nameCSC)[0]
...
for i in range(0,int(dataCSC.shape[0]/rate)):
	...
	if(pred > 0.75):#victor
		sf.write('Victor/'+str(i)+'.wav', dataCSC[i*rate:(i+1)*rate], 22000, 'PCM_24')
		# librosa.output.write_wav('Victor/'+str(i)+'.wav', dataCSC[i*rate:(i+1)*rate])
	if(pred < 0.25):#Teo
		sf.write('Teo/'+str(i)+'.wav', dataCSC[i*rate:(i+1)*rate], 22000, 'PCM_24')
		# librosa.output.write_wav('Teo/'+str(i)+'.wav', dataCSC[i*rate:(i+1)*rate])
	if(pred >= 0.25 and pred <=0.75):#victor
		sf.write('Nesigur/'+str(i)+'.wav', dataCSC[i*rate:(i+1)*rate], 22000, 'PCM_24')
